...
function get_siteemail() {
  return 'sedemalocal@localhost.com';
}
...
function get_name_user() {
	return get_user('nombre').' '. get_user('apellido_p');
}
 ...
 * @return void
 */
// function send_email($from, $to, $subject, $body, $alt = null) {
// 	$mail     = new PHPMailer();
...
// 	$mail->msgHTML(get_module($template, ['alt' => $alt, 'body' => $body, 'subject' => $subject]));
// 	//$mail->msgHTML(get_module($template, ['alt' => $alt, 'body' => $body, 'subject' => $subject]));
// 	$mail->AltBody = $alt;
...
function get_favicon() {
	...
	$favicon     = 'logo.ico'; // nombre del archivo favicon
	$type        = '';
	$href        = '';
...
function base64_image($img_file)
{
